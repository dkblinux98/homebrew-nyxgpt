class NyxgptApiAT300rc < Formula
  desc "Release candidate 3.0.0rc14 -- nyxGPT FastAPI backend (uvicorn)"
  ...
  url "https://github.com/dkblinux98/nyxGPT/releases/download/3.0.0rc14/nyxgpt-api-3.0.0rc14.tar.gz"
  sha256 "7f44d8628a2436a50a1b19f12c91f440ebd86ad7a3a7ab7f8b9cf4999e75b4cd"
  version "3.0.0rc14"
  license "MIT"

  # Acceptance-only channel (#3727): `brew install nyxgpt-api` must always
  ...
  #
  # A tap that does not (yet) carry nyxgpt-api.rb makes brew warn that the
  ...
  #
  # The mirror of this lives on nyxgpt-api since #3853 -- one direction
  # declared is not a guard, it is a guard on one of two orders.
  conflicts_with "nyxgpt-api",
    because: "both install the same nyxgpt-api wrapper and brew service"
...
PY
  ...
  test do
    ...
    # Exactly the keg's own version (#3850, re-test). This is the formula the
    # owner actually installs, and the one whose rc13 keg reported the stable
    # 3.0.0 and sent `nyxgpt up` to the wrong channel. See ../nyxgpt-api.rb's
    # test block for the account.
